...
class TgUploader:
    ...
    async def __prepare_file(self, file_, dirpath):
        if self.__lprefix:
            ...
            if self.__listener.seed and not self.__listener.newDir and not dirpath.endswith("/splited_files_server0x01"):
                dirpath = f'{dirpath}/copied_server0x01'
                ...
                try:
                    ...
                    if not self.__is_cancelled and await aiopath.exists(self.__up_path) and \
                        (not self.__listener.seed or self.__listener.newDir or
                         dirpath.endswith("/splited_files_server0x01") or '/copied_server0x01/' in self.__up_path):
                        await aioremove(self.__up_path)
        for key, value in list(self.__media_dict.items()):
            for subkey, msgs in list(value.items()):
                if len(msgs) > 1:
